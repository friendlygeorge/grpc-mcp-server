...
export function registerConnectTools(server, ctx) {
    server.tool("connect_endpoint", "Connect to a gRPC server endpoint and discover available services via server reflection", {
        endpoint: z
            .string()
        ...
        try {
            ...
            const serviceList = Array.from(services.values())
                .map((s) => `• **${s.name}** — ${s.methods.length} method(s): ${s.methods.map((m) => m.name).join(", ")}`)
                .join("\n");
            return {
                content: [
                    {
                        type: "text",
                        text: services.size > 0
    ...
    server.tool("disconnect", "Disconnect from the current gRPC endpoint and clear cached service data", {}, async () => {
        ctx.disconnect();
        return {
            content: [
