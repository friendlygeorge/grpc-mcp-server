...
async function main() {
    ...
    if (ctx.isConfigured()) {
        process.stderr.write(`gRPC MCP server ready — endpoint: ${ctx.getEndpoint()}\n`);
    }
    else {
        process.stderr.write(`gRPC MCP server ready — no endpoint configured. Use connect_endpoint or set GRPC_ENDPOINT.\n`);
    }
